...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      double velocity = measurement_pack.raw_measurements_[2];
      
      double x_cart = range * cos(bearing);
      double y_cart = range * sin(bearing);
      double xv_cart = velocity * cos(bearing);
      double yv_cart = velocity * sin(bearing);
      
      ekf_.x_ << x_cart, y_cart, xv_cart , yv_cart;
    }
